...
else 
{
    ...
    else
    {
        ...
        if($role == "admin")
        {
            $stmt = $conn->prepare("INSERT INTO Public_Events (EventID, AdminID) VALUES (?, ?)");
            $stmt->bind_param("ii", $eventID, $userID);
        }
        else
        {
            $stmt = $conn->prepare("INSERT INTO Public_Events (EventID, SuperAdminID) VALUES (?, ?)");
            $stmt->bind_param("ii", $eventID, $userID);
        }
        
        if (!$stmt->execute()) {
            returnWithError("Error inserting into Public_Events: " . $stmt->error, 500);
            return;
        }
